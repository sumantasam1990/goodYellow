...
export class Step2Component implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.http.get(this.oneUrl + '1').subscribe(res => {
      this.dataOne = res
    });
  ...
  select_changeTwo() {
    if(this.b_lb_two == 'new_00_two') {
      ...
      this.http.get(this.threeUrl + this.b_lb_two + '/' + '1').subscribe(res => {
        this.dataThree = res
      });
    }
  }
  ...
  async getCategories() {
    await this.http.get('https://administrator.goodyellowco.com/api/u/vendor/category/' + localStorage.getItem('u_id') + '?token=' + localStorage.getItem('token')).pipe(delay(100), retry(3)).toPromise().then(res => {
      this.categories = res
    });
  }
    ...
    if(status === 0) {
      ...
      if(this.b_lb_three == '' || this.b_lb_three == ' ') {
        ...
        if(status === 3) {
          ...
          formData.append('user_id', this.uid ?? '')
          formData.append('category_id_main', '1')
        }
