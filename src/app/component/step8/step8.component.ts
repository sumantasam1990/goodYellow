...
export class Step8Component implements OnInit {
  ...
  dataUrl: any = 'https://administrator.goodyellowco.com/api/u/vendor/brand/video/'
  ...
  next() {
    const myheader = new HttpHeaders();
      ...
      formData.append('key', 'brand_video')
      ...
      this.http.post<any>(this.url + this.token, formData, {
        ...
        if(response) {
          this.txt = ''
          localStorage.setItem('goodyellow_step', '8')
          this.router.navigate(['vendor-signup-links'])
        }
      });
  }
